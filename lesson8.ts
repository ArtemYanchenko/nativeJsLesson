...
export const isEvenIndexSumGreater = (arr: Array<number>) => {
    //...здесь пишем код.
    // В return стоит "заглушка", чтоб typescript не ругался
    const even = arr.filter((el, index) => index % 2 === 0).reduce((acc, el) => acc + el, 0)
    const odd = arr.filter((el, index) => index % 2 !== 0).reduce((acc, el) => acc + el, 0)
    return even > odd
...
export function getSquarePositiveIntegers(array: Array<number>): Array<number> {
    ...
    return array.filter(el => el % 1 === 0 && el > 0).map(el => Math.pow(el, 2))
}
